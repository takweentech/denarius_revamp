...
export class PaymentComponent extends BaseComponent implements OnInit {
  ...
  opportunity: Opportunity =
    this.activatedRoute.parent?.snapshot.data["opportunity"]?.data;
  investment = signal<InvestmentResponse | null>(null);
  WEB_ROUTES = WEB_ROUTES;
  numStock: FormControl<number | null> = new FormControl(
    0,
    Validators.required
  );
  terms: FormControl<boolean | null> = new FormControl(
    false,
