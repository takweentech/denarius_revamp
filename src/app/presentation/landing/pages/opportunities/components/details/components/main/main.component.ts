...
@Component({
  selector: 'app-main',
  imports: [DecimalPipe, TranslateModule, NgbAccordionModule, NgClass, ReactiveFormsModule],
...
export class MainComponent extends BaseComponent {
  ...
  onSignIn(): void {
    this.router.navigate(['/' + WEB_ROUTES.AUTH.ROOT + '/' + WEB_ROUTES.AUTH.SIGN_IN], {
      state: { redirectionUrl: this.router.url },
    });
  }

  onSignUp(): void {
    this.router.navigate(['/' + WEB_ROUTES.AUTH.ROOT + '/' + WEB_ROUTES.AUTH.SIGN_UP], {
      state: { redirectionUrl: this.router.url },
    });
  }
