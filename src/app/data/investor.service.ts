...
export class InvestorService {
    ...
    getPaged(filter?: InvestmentFilter): Observable<HttpCustomResponse<HttpPagedResponse<Investment[]>>> {
        return this.http.post<HttpCustomResponse<HttpPagedResponse<Investment[]>>>(`${environment.apiUrl}/${this.baseUrl}/GetInvestorOpportunitiesPaged`, filter);
    }

    getById(id: number): Observable<HttpCustomResponse<Investment>> {
        return this.http.get<HttpCustomResponse<Investment>>(`${environment.apiUrl}/${this.baseUrl}/InvestorOpportunityDetails${id}`)
    }
}
